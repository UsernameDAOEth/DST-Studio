...
- **Signal Generation**: Identifying potential trade setups across multiple timeframes (4H via DefiLlama, 15m via Pyth Benchmarks TradingView shim).
...
- **Multi-timeframe signals (additive, 4H byte-identical)**: Hermes scans every (asset × timeframe) pair on each trigger. The 4H pipeline keeps DefiLlama as its sole historical-bar source — byte-identical to the previous behaviour. The 15m pipeline is powered by `getHistorical15mWithProvenance` (`artifacts/api-server/src/lib/pyth/benchmarksClient.ts`), a thin wrapper over the public, key-free Pyth Benchmarks TradingView shim (`https://benchmarks.pyth.network/v1/shims/tradingview/history`); on failure it sets the new `PYTH_BENCHMARKS_UNAVAILABLE` quality flag and degrades to WAIT. The current price always comes from DefiLlama (cross-source verification); only the historical close series differs by timeframe. `prev24Price` lookback is timeframe-aware (6 bars for 4H, 96 bars for 15m). Each (asset, timeframe) pair persists as its own `signals` row (DB column `timeframe` defaults to "4H") and is independently audited. Telegram + AgentMail dedup keys are now `${tf}:${asset}:${packetHash}` and message subjects/headers carry the timeframe label. Dashboard exposes a 4H/15m toggle that propagates to `/api/signals?timeframe=…`, `/api/signals/feed?timeframe=…`, `/api/dst/pipeline-health?timeframe=…`, and the SHORT bottleneck endpoints; signal-detail and audit pages read the timeframe from the URL query.
- **Pyth Client**: Integrates with Hermes REST API for secondary price-confidence verification.
- **Pyth Lazer Stream** (additive): Singleton WebSocket client (`artifacts/api-server/src/lib/dst/lazerClient.ts`) subscribes to BTC/ETH/SOL via `@pythnetwork/pyth-lazer-sdk` (channel `fixed_rate@200ms`). Latest tick per asset held in memory only — no DB persistence. Exposed via `GET /api/lazer/snapshot` and rendered on dashboard by `LazerStreamPanel`. When `PYTH_LAZER_API_KEY` is set and the stream is `CONNECTED`, `getHistorical15mWithProvenance` overlays the latest tick (≤15s old) onto the in-flight 15m bar's close so 15m signal generation sees sub-second freshness without re-fetching the Benchmarks shim each scan; cache TTL bumps from 45s to 14min in this mode and provenance source switches to `PYTH_LAZER_STREAM`. When the stream is `UNCONFIGURED`/`DISCONNECTED`/`ERROR`, the original 45s polling cache + Benchmarks-only provenance is preserved (zero behaviour change). Does NOT feed into DJZS audit logic itself — only the indicator close series.
...
- **Data Providers**:
    ...
    - Pyth Benchmarks TradingView shim (free, no key — backs the 15m timeframe historical bar feed)
